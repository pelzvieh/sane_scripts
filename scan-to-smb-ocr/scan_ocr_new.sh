...
inotifywait --monitor -e close_write $in_base | awk '/.*CLOSE_WRITE.*/ {
    ...
    system("openssl ts -query -data /var/spool/scanbd/" $3 " -cert -sha256 -out - | curl -s -S -H '\''Content-Type: application/timestamp-query'\'' --data-binary @- http://zeitstempel.dfn.de/ -o /var/spool/scanbd/" $3 ".tsr");
}'
